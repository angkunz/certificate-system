    ...
    <div ref={scalerRef} style={{ width: '100%', maxWidth: 842, aspectRatio: '842/595', position: 'relative', overflow: 'hidden', borderRadius: 8 }}>
      ...
      >
        ...
        <div style={{ ...el('logo'), ...hidden('logo') }}
          ...
          {org.logo_url
            ? <img src={org.logo_url} alt="logo" crossOrigin="anonymous"
                style={{ height: `calc(42px * ${sz('logo')})`, width: 'auto', objectFit: 'contain', display: 'block' }} />
            : <div style={{ width: `calc(42px * ${sz('logo')})`, height: `calc(42px * ${sz('logo')})`, borderRadius: 8, background: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: `calc(22px * ${sz('logo')})`, color: 'white' }}>🏛️</div>
          }
        </div>

        <div style={{ ...el('orgName'), ...hidden('orgName'), ...textStyle('orgName', 'rgba(255,255,255,0.95)'), fontWeight: textStyle('orgName', '').fontWeight ?? 700, fontSize: `calc(13px * ${sz('orgName')})`, maxWidth: 336, lineHeight: 1.3 }}
          onMouseDown={startDrag('orgName')} onTouchStart={startDrag('orgName')}>
          {org.name}
        </div>

        <div style={{ ...el('title'), ...hidden('title'), fontFamily: "'Playfair Display','Sarabun',serif", fontSize: `calc(32px * ${sz('title')})`, fontWeight: textStyle('title', '').fontWeight ?? 700, letterSpacing: '0.08em', textShadow: '0 2px 8px rgba(0,0,0,0.4)', whiteSpace: 'nowrap', ...textStyle('title', '#fcd34d') }}
          onMouseDown={startDrag('title')} onTouchStart={startDrag('title')}>
          เกียรติบัตร
        </div>

        <div style={{ ...el('presentText'), ...hidden('presentText'), fontSize: `calc(11px * ${sz('presentText')})`, whiteSpace: 'nowrap', ...textStyle('presentText', 'rgba(255,255,255,0.8)') }}
          onMouseDown={startDrag('presentText')} onTouchStart={startDrag('presentText')}>
          มอบเกียรติบัตรฉบับนี้ให้แก่
        </div>

        <div style={{ ...el('recipient'), ...hidden('recipient'), fontFamily: "'Playfair Display','Sarabun',serif", fontSize: `calc(24px * ${sz('recipient')})`, fontWeight: textStyle('recipient', '').fontWeight ?? 700, textShadow: '0 2px 8px rgba(0,0,0,0.4)', whiteSpace: 'nowrap', maxWidth: 673, overflow: 'hidden', textOverflow: 'ellipsis', ...textStyle('recipient', 'white') }}
          onMouseDown={startDrag('recipient')} onTouchStart={startDrag('recipient')}>
          {recipient.full_name}
        </div>

        {recipient.award && (
          <div style={{ ...el('award'), ...hidden('award'), fontSize: `calc(12px * ${sz('award')})`, fontWeight: textStyle('award', '').fontWeight ?? 600, whiteSpace: 'nowrap', maxWidth: 589, overflow: 'hidden', textOverflow: 'ellipsis', ...textStyle('award', '#fcd34d') }}
            onMouseDown={startDrag('award')} onTouchStart={startDrag('award')}>
            {recipient.award}
          </div>
        )}

        <div style={{ ...el('divider'), ...hidden('divider'), width: `calc(68px * ${sz('divider')})`, height: 2, background: layout.divider.color ? `linear-gradient(90deg,transparent,${layout.divider.color},transparent)` : 'linear-gradient(90deg,transparent,#fcd34d,transparent)' }}
          onMouseDown={startDrag('divider')} onTouchStart={startDrag('divider')} />

        <div style={{ ...el('activity'), ...hidden('activity'), fontSize: `calc(14px * ${sz('activity')})`, fontWeight: textStyle('activity', '').fontWeight ?? 700, whiteSpace: 'nowrap', maxWidth: 673, overflow: 'hidden', textOverflow: 'ellipsis', ...textStyle('activity', '#fcd34d') }}
        ...
        {(recipient.extra_details || recipient.activity?.description) && (
          <div style={{ ...el('description'), ...hidden('description'), fontSize: `calc(10px * ${sz('description')})`, maxWidth: 589, lineHeight: 1.5, ...textStyle('description', 'rgba(255,255,255,0.75)') }}
        ...
        {certDate && (
          <div style={{ ...el('date'), ...hidden('date'), fontSize: `calc(10px * ${sz('date')})`, whiteSpace: 'nowrap', ...textStyle('date', 'rgba(255,255,255,0.75)') }}
        ...
        <div style={{ ...el('signature'), ...hidden('signature'), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}
          ...
          {org.signature_url && (
            <img src={org.signature_url} alt="sig" crossOrigin="anonymous"
              style={{ height: `calc(34px * ${sz('signature')})`, objectFit: 'contain', filter: 'brightness(0) invert(1)', marginBottom: 2 }} />
          )}
          <div style={{ width: `calc(84px * ${sz('signature')})`, height: 1, background: 'rgba(255,255,255,0.5)' }} />
          <div style={{ fontSize: `calc(10px * ${sz('signature')})`, fontWeight: 700, whiteSpace: 'nowrap', marginTop: 2, ...textStyle('signature', 'white') }}>
            {org.executive_name}
          </div>
          <div style={{ fontSize: `calc(9px * ${sz('signature')})`, whiteSpace: 'nowrap', color: layout.signature.color || 'rgba(255,255,255,0.7)' }}>
        ...
        {qrDataUrl && (
          <div style={{ ...el('qr'), ...hidden('qr'), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}
            ...
            <div style={{ fontSize: `calc(8px * ${sz('qr')})`, color: 'rgba(255,255,255,0.6)', whiteSpace: 'nowrap' }}>
              ตรวจสอบเกียรติบัตร
            </div>
            <div style={{ background: 'white', borderRadius: 6, padding: 4, width: `calc(50px * ${sz('qr')})`, height: `calc(50px * ${sz('qr')})`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <img src={qrDataUrl} alt="qr" style={{ width: '100%', height: '100%' }} />
            </div>
            <div style={{ fontSize: `calc(6px * ${sz('qr')})`, color: 'rgba(255,255,255,0.5)', fontFamily: 'monospace', maxWidth: `calc(50px * ${sz('qr')})`, wordBreak: 'break-all', textAlign: 'center' }}>
              {recipient.cert_code}
            </div>
          </div>
        )}
        ...
        {editMode && (
          <div style={{ position: 'absolute', bottom: '2%', left: '50%', transform: 'translateX(-50%)', background: 'rgba(0,0,0,0.75)', color: 'white', fontSize: '9px', padding: '3px 12px', borderRadius: 20, whiteSpace: 'nowrap', pointerEvents: 'none', zIndex: 20 }}>
